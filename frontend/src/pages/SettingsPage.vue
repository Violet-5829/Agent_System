...
const sqlProbeResult = ref("");
const sqlForm = reactive({ name: "", dialect: "sqlite", connection_string: "" });
const excelForm = reactive({ name: "", file_path: "" });
...
async function addSqlSource() {
  if (!sqlForm.name || !sqlForm.connection_string) return;
  try {
    await createSqlDataSource({ ...sqlForm, read_only: true });
    sqlForm.name = ""; sqlForm.connection_string = ""; sqlForm.dialect = "sqlite";
    showSqlForm.value = false;
    await loadDataSources();
  } catch(e) { alert(e.message); }
}

async function removeSqlSource(id) {
  try { await deleteSqlDataSource(id); await loadDataSources(); } catch(e) { alert(e.message); }
}

async function probeSql(id) {
  try {
    const r = await probeSqlDataSource(id);
    sqlProbeResult.value = `表: ${(r.tables || []).join(", ") || "无"}`;
  } catch(e) { sqlProbeResult.value = `错误: ${e.message}`; }
}

async function addExcelDataset() {
  if (!excelForm.name || !excelForm.file_path) return;
  try {
    await createExcelDataset({ ...excelForm });
    excelForm.name = ""; excelForm.file_path = "";
    showExcelForm.value = false;
    await loadDataSources();
  } catch(e) { alert(e.message); }
}

async function removeExcelDataset(id) {
  try { await deleteExcelDataset(id); await loadDataSources(); } catch(e) { alert(e.message); }
}
  ...
  <section class="page-stack settings-page">
    ...
    <section class="glass-panel section-card settings-card">
      <div class="section-header">
        <div>
          ...
          <p>管理 SQL 数据库连接（SQLite / PostgreSQL）</p>
        </div>
        <button type="button" class="text-button" @click="showSqlForm = true">
          <Plus :size="14" />
          添加
        </button>
      </div>

      <div v-if="sqlSources.length" class="settings-env-list">
        <article v-for="s in sqlSources" :key="s.id" class="settings-env-item">
          <div class="settings-env-summary">
            <div class="settings-env-copy">
              <div class="settings-env-key">{{ s.name }}</div>
              <div class="settings-env-value">{{ s.dialect }} · 只读: {{ s.read_only ? '是' : '否' }}</div>
            </div>
            <div class="settings-env-actions">
              <button type="button" class="settings-profile-icon-action" @click="probeSql(s.id)">探测</button>
              <button type="button" class="settings-profile-icon-action" @click="removeSqlSource(s.id)">
                <Trash2 :size="16" />
              </button>
            </div>
          </div>
      ...
      <div v-else class="agent-empty-meta">暂无 SQL 数据源</div>

      <div v-if="sqlProbeResult" style="font-size:12px;margin-top:8px;color:#475569">{{ sqlProbeResult }}</div>

      <div v-if="showSqlForm" class="settings-env-editor" style="margin-top:12px">
        <div class="settings-env-row">
          <input v-model="sqlForm.name" placeholder="名称" />
          <select v-model="sqlForm.dialect" style="padding:8px 12px;border:1px solid #d1d5db;border-radius:8px">
            <option value="sqlite">SQLite</option>
            <option value="postgresql">PostgreSQL</option>
          </select>
        </div>
        <div class="settings-env-row" style="margin-top:8px">
          <input v-model="sqlForm.connection_string" placeholder="连接串（如 /path/to/db.sqlite）" style="flex:1" />
        </div>
        <div class="settings-env-editor-actions" style="margin-top:8px">
          <button type="button" class="settings-env-save" @click="addSqlSource"><Save :size="14" /> 保存</button>
          <button type="button" class="ghost-button" @click="showSqlForm = false"><X :size="14" /> 取消</button>
        </div>
      </div>
    </section>
    ...
    <section class="glass-panel section-card settings-card">
      <div class="section-header">
        <div>
          <h3>
            <FileSpreadsheet :size="18" class="text-slate-400" />
            Excel 数据集
          </h3>
          <p>管理 Excel/CSV 文件（文件需先放入 data/excel_inbox/ 目录）</p>
        </div>
        <button type="button" class="text-button" @click="showExcelForm = true">
          <Plus :size="14" />
          注册
        </button>
      </div>

      <div v-if="excelDatasets.length" class="settings-env-list">
        <article v-for="d in excelDatasets" :key="d.id" class="settings-env-item">
          <div class="settings-env-summary">
            <div class="settings-env-copy">
              <div class="settings-env-key">{{ d.name }}</div>
              <div class="settings-env-value" style="font-family:monospace">{{ d.file_path }}</div>
            </div>
            <div class="settings-env-actions">
              <button type="button" class="settings-profile-icon-action" @click="removeExcelDataset(d.id)">
                <Trash2 :size="16" />
              </button>
            </div>
          </div>
      ...
      <div v-else class="agent-empty-meta">暂无 Excel 数据集</div>

      <div v-if="showExcelForm" class="settings-env-editor" style="margin-top:12px">
        <div class="settings-env-row">
          <input v-model="excelForm.name" placeholder="名称" />
          <input v-model="excelForm.file_path" placeholder="相对路径（如 data.csv）" />
        </div>
        <div class="settings-env-editor-actions" style="margin-top:8px">
          <button type="button" class="settings-env-save" @click="addExcelDataset"><Save :size="14" /> 保存</button>
          <button type="button" class="ghost-button" @click="showExcelForm = false"><X :size="14" /> 取消</button>
        </div>
      </div>
    </section>
  </section>
